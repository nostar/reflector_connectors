/*
    XrfCon - DExtra Reflector Connector
...
void process_signal(int sig)
{
	...
	if(sig == SIGINT){
		...
		sendto(udp2, buf, 11, 0, (const struct sockaddr *)&host2, sizeof(host2));
#ifdef DEBUG_SEND
...
int main(int argc, char **argv)
{
	...
	if(argc != 4){
		fprintf(stderr, "Usage: xrfcon [CALLSIGN] [XRFName1:MOD1:XRFHost1IP:PORT] [XRFName2:MOD2:XRFHost2IP:PORT]\n");
		return 0;
	}
	else{
		memset(callsign, ' ', 8);
	...
	while (1) {
		...
		if(rxlen == 9){ //keep-alive
			memcpy(buf, callsign, 8);
			buf[8] = 0x00;
			sendto(udprx, buf, 9, 0, (const struct sockaddr *)&rx, sizeof(rx));
		...
		if((rxlen == 0x38) && (!memcmp(&buf[0], header, 4))) { //dv header
			for(int i = 0; i < 4; ++i){
				if((buf[0x32+i] > 0x20) && (buf[0x32+i] < 0x30)){
					buf[0x32+i] = 0x20;
				}
				else if((buf[0x32+i] > 0x39) && (buf[0x32+i] < 0x41)){
					buf[0x32+i] = 0x20;
				}
				else if((buf[0x32+i] > 0x5a) && (buf[0x32+i] < 0x61)){
					buf[0x32+i] = 0x20;
				}
				else if(buf[0x32+i] > 0x7a) {
					buf[0x32+i] = 0x20;
				}
			}
			
			if( (udprx == udp1) && (rx.sin_addr.s_addr == host1.sin_addr.s_addr) && !memcmp(&buf[18], ref1, 6) && (buf[25] == mod1[0]) ){
				memcpy(&buf[18], ref2, 6);
				buf[24] = ' ';
				buf[25] = mod2[0];
				streamid = (buf[12] << 8) | (buf[13] & 0xff);
				addCCITT161(&buf[15], 41);
				sendto(udp2, buf, 0x38, 0, (const struct sockaddr *)&host2, sizeof(host2));
#ifdef DEBUG_SEND
				fprintf(stderr, "SEND %s: ", ref2);
			...
			else if( (udprx == udp2) && (rx.sin_addr.s_addr == host2.sin_addr.s_addr) && !memcmp(&buf[18], ref2, 6) && (buf[25] == mod2[0]) ){
				memcpy(&buf[18], ref1, 6);
				buf[24] = ' ';
				buf[25] = mod1[0];
				streamid = (buf[12] << 8) | (buf[13] & 0xff);
				addCCITT161(&buf[15], 41);
				sendto(udp1, buf, 0x38, 0, (const struct sockaddr *)&host1, sizeof(host1));
#ifdef DEBUG_SEND
				fprintf(stderr, "SEND %s: ", ref1);
				for(int i = 0; i < 0x38; ++i){
					fprintf(stderr, "%02x ", buf[i]);
		...
		if(rxlen == 0x1b){ //dv frame
			uint16_t s = (buf[12] << 8) | (buf[13] & 0xff);
			if(s == streamid){
				if( (udprx == udp1) && (rx.sin_addr.s_addr == host1.sin_addr.s_addr) ){
					sendto(udp2, buf, 0x1b, 0, (const struct sockaddr *)&host2, sizeof(host2));
#ifdef DEBUG_SEND
					fprintf(stderr, "SEND %s: ", ref2);
					...
					sendto(udp1, buf, 0x1b, 0, (const struct sockaddr *)&host1, sizeof(host1));
#ifdef DEBUG_SEND
					fprintf(stderr, "SEND %s: ", ref1);
					for(int i = 0; i < 0x1b; ++i){
						fprintf(stderr, "%02x ", buf[i]);
